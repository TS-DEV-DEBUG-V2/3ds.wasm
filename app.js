...
async function detectAzaharRuntimeFormat(src) {
  ...
  azaharRuntimeFormatPromise = (async () => {
    ...
    while (totalLength < AZAHAR_RUNTIME_SNIFF_LIMIT) {
      ...
      const snippet = decoder.decode(value, {
        stream: true
      });
...
const state = {
  ...
  experimentalHardwareShaders: rendererPreset === "webgl-full" ||
...
function get2dContext() {
  if (!state.canvas2dContext) {
    state.canvas2dContext =
      elements.softwareCanvas?.getContext("2d", {
        alpha: false
      }) || null;
  }
...
(async function autoBootRom() {
  try {
    window.dispatchEvent(new CustomEvent("rom-progress", { detail: { phase: "core" } }));
    await loadCore();
    await wakeAudioContext().catch(() => {});

    window.dispatchEvent(new CustomEvent("rom-progress", { detail: { phase: "fetch", loaded: 0, total: 0 } }));
    const response = await fetch(HARDCODED_ROM_URL);
    ...
    while (true) {
      ...
      chunks.push(value);
      loaded += value.length;
      window.dispatchEvent(new CustomEvent("rom-progress", { detail: { phase: "fetch", loaded, total: contentLength } }));
    }
    ...
    window.dispatchEvent(new CustomEvent("rom-progress", { detail: { phase: "boot" } }));
    await bootRomData(HARDCODED_ROM_NAME, bytes);
    window.dispatchEvent(new CustomEvent("rom-progress", { detail: { phase: "done" } }));
  } catch (error) {
    window.dispatchEvent(new CustomEvent("rom-progress", { detail: { phase: "error", message: error.message } }));
    reportRuntimeFailure("Auto-boot failed", error);
  }
})();
